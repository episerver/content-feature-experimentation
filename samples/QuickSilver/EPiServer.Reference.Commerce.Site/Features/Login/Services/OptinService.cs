...
using EPiServer.Reference.Commerce.Site.Infrastructure.Identity;
...
namespace EPiServer.Reference.Commerce.Site.Features.Login.Services
{
    ...
    public class OptinService : IDisposable
    {
        ...
        public virtual async Task<string> CreateOptinTokenData(string email)
        {
            ...
            var token = await _userManager.GenerateUserTokenAsync(user, OptinTokenProviderOptions.TokenProviderName, MarketingEmailOptinPurpose);
        ...
        public async Task<bool> ConfirmOptinToken(string email, string token)
        {
            ...
            var confirmResult = await _userManager.VerifyUserTokenAsync(user, OptinTokenProviderOptions.TokenProviderName, MarketingEmailOptinPurpose, token);
            if (!confirmResult)
            {
                return false;
            }
