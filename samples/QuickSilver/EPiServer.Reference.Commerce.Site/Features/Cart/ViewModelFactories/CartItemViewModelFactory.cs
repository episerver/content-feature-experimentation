...
namespace EPiServer.Reference.Commerce.Site.Features.Cart.ViewModelFactories
{
    ...
    public class CartItemViewModelFactory
    {
        ...
        {
            return product != null ?
                _catalogContentService.GetVariants<FashionVariant>(product).Where(x => string.Compare(x.Color, entry.Color, StringComparison.InvariantCultureIgnoreCase) == 0).Select(x => x.Size)
