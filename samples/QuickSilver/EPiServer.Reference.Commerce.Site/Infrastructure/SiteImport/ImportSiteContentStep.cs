...
namespace EPiServer.Reference.Commerce.Site.Infrastructure.SiteImport
{
    ...
    public class ImportSiteContentStep : IMigrationStep
    {
        ...
        private Injected<IContentRepository> _contentRepository = default;
        private Injected<ReferenceConverter> _referenceConverter = default;
        private Injected<IDataImporter> _dataImporter = default;
        private Injected<TaxImportExport> _taxImportExport = default;
        private Injected<IPaymentManagerFacade> _paymentManager = default;
        ...
        private void ConfigurePaymentMethods()
        {
            ...
            foreach (var language in existingLanguages)
            {
                ...
                AddPaymentMethod(Guid.NewGuid(),
                    ...
                    false, 1, allMarkets, language, workingDto);

                AddPaymentMethod(Guid.NewGuid(),
                    "Adyen Payment",
                    "Adyen",
                    "Adyen Payment methods implementation.",
                    "EPiServer.Reference.Commerce.Site.Features.Payment.PaymentMethods.AdyenPayment, EPiServer.Reference.Commerce.Site",
                    "EPiServer.Reference.Commerce.Site.Features.Payment.PaymentMethods.AdyenPaymentGateway, EPiServer.Reference.Commerce.Site",
        ...
        private IEnumerable<ShippingMethodDto.ShippingMethodRow> CreateShippingMethodsForLanguageAndCurrencies(ShippingMethodDto dto, IEnumerable<IMarket> markets, string languageId)
        {
            ...
            {
                shippingMethods.Add(CreateShippingMethod(dto, shippingOption, languageId, sortOrder++, "Express_" + currency, $"Express {currency} (1 day)({languageId})", usdCostExpress, currency));
                shippingMethods.Add(CreateShippingMethod(dto, shippingOption, languageId, sortOrder++, "Fast_" + currency, $"Fast {currency} (2-3 days)({languageId})", usdCostFast, currency));
                shippingMethods.Add(CreateShippingMethod(dto, shippingOption, languageId, sortOrder++, "Regular_" + currency, $"Regular {currency} (4-7 days)({languageId})", usdCostRegular, currency));
            }
...
#pragma warning disable CS0618 // Type or member is obsolete
            _objectInstanceCache.Service.Clear();
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static void SyncMetaClassesToContentTypeModels()
        {
            ...
            tasks.AddRange(contentScanner.Sync(ServiceLocator.Current.GetInstance<ContentModelOptions>().EnableModelSyncCommit));
